...
def main(argv) -> int:
    ...
    utils.clear_con()
    print(colorama.Fore.LIGHTBLUE_EX + f"Название: {colorama.Fore.LIGHTYELLOW_EX}{anime_data['title']}")
    print(colorama.Fore.LIGHTBLUE_EX + f"Теги: {colorama.Fore.LIGHTCYAN_EX}{', '.join(anime_data['tags'])}")

    print(colorama.Fore.LIGHTBLACK_EX + "\nСезоны: ")
    for season in anime_data["seasons"]:
        print(colorama.Fore.LIGHTGREEN_EX + f" {season}) {len(anime_data['seasons'][season])} эпизодов.")
    ...
    if select.upper()[0] == "S":
        ...
        from_ = int(input(colorama.Fore.LIGHTBLUE_EX + f"\nС какой серии качать (Максимум: {max_}): "))
        to_ = int(input(colorama.Fore.LIGHTRED_EX + f"До какой серии качать (Максимум: {max_}): "))
        quality_ = input(colorama.Fore.LIGHTYELLOW_EX + f"Качество видео (360, 480, 720, 1080): ")
        ...
        utils.clear_con()
        print(colorama.Fore.LIGHTGREEN_EX + "Ход работы: \n" + colorama.Fore.LIGHTCYAN_EX)

        download_log = "JutSu Download Log\n"

        for src in to_download:
            try:
                code = src.download(utils.session, f"{path}/{src.title} (Эпизод {src.ep}).mp4")

                if code == "0":
                    download_log += "\n" + f"[{path}/{src.title} (Эпизод {src.ep}).mp4] - " + "Ошибка скачивания (invalid session)"
                else:
                    download_log += "\n" + f"[{path}/{src.title} (Эпизод {src.ep}).mp4] - " + "Успешно!"
            except Exception as err:
                download_log += "\n" + f"[{path}/{src.title} (Эпизод {src.ep}).mp4] - " + f"Ошибка скачивания ({err})"

        log_file = open(path+"/download.log", "w", encoding="utf-8")
